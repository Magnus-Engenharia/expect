...
export class PlanningError extends Schema.ErrorClass<PlanningError>("@supervisor/PlanningError")({
  ...
  reason: Schema.Union([AgentStreamError, PlanParseError]),
}) {
  message = `Planning failed: ${this.reason.message}`;
}

const extractText = (event: ExecutionEvent): Result.Result<string, ExecutionEvent> =>
  event._tag === "AgentText" ? Result.succeed(event.text) : Result.fail(event);

const PLANNER_MAX_ATTEMPTS = 3;

const parsePlanFile = Effect.fn("Planner.parsePlanFile")(function* (
  sentinelPath: string,
  draft: TestPlanDraft,
) {
