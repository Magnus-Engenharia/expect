...
const actOnPage = (
  ...
  Effect.gen(function* () {
    const browser = yield* Browser;
    return yield* browser.act(page, ref, action, options);
  }).pipe(Effect.provide(Browser.layer));
...
describe("act", () => {
  ...
  it("should perform a click action and return updated snapshot", async () => {
    ...
    const before = await run(snapshotPage(page));
    const buttonRef = Object.keys(before.refs).find((key) => before.refs[key].name === "Click Me");
    expect(buttonRef).toBeDefined();

    const after = await run(actOnPage(page, buttonRef!, (locator) => locator.click()));
  ...
  it("should perform a fill action and return updated snapshot", async () => {
    ...
    const after = await run(actOnPage(page, inputRef!, (locator) => locator.fill("Alice")));
  ...
  it("should toggle a checkbox via act", async () => {
    ...
    await run(actOnPage(page, checkboxRef!, (locator) => locator.check()));
  ...
  it("should return a snapshot with valid refs after action", async () => {
    ...
    const locator = await run(after.locator(linkRef!));
  ...
  it("should forward snapshot options", async () => {
    ...
    const interactiveBefore = await run(snapshotPage(page, { interactive: true }));
    ...
    const after = await run(
      actOnPage(page, buttonRef!, (locator) => locator.click(), { interactive: true }),
    );
  ...
  it("should handle actions on duplicate-named elements", async () => {
    ...
    await run(actOnPage(page, goButtons[1][0], (locator) => locator.click()));
