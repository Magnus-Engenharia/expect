...
export const buildExecutionPrompt = (options: ExecutionPromptOptions): string => {
  ...
  return [
    ...
    "6. accessibility_audit — Run a WCAG accessibility audit (axe-core + IBM Equal Access). Returns violations sorted by severity with selectors, HTML context, and fix guidance. Call after opening a page or completing a significant interaction.",
    "7. close — Close the browser and end the session.",
    "",
    ...
    "After completing the primary flow, call accessibility_audit once to check for WCAG violations. Report critical or serious violations that relate to the changed code as assertion failures.",
    "",
